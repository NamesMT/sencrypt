...
Releases are version-first and dispatched by hand: one workflow run does the whole release, so a
`git push` on its own never publishes anything.
...
2. [`.github/workflows/release.yml`](.github/workflows/release.yml) verifies the version, runs
   `pnpm run check` and the build, then lets [changelogen](https://github.com/unjs/changelogen)
...
One-time setup: publish the package once by hand (npm only offers a trusted publisher for a package
that already exists), then on npmjs.com enable **Settings → Publishing access → Trusted Publishing**
for `namesmt/sencrypt` with the workflow filename `release.yml`.
